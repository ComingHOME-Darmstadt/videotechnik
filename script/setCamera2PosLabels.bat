...
set cfgFileNew=%cfgPath%camera2Labels-new.cfg

setlocal ENABLEDELAYEDEXPANSION

echo.
echo.soll ein Positions-Label fÅr Kamera 2 beibehalten werden, einfach mit Return bestÑtigen.
echo.
pause
echo.
...
for /F "tokens=1,2 delims==" %%i in (%cfgFile%) do (
  ...
  curl -s --request POST http://127.0.0.1:8888/api/custom-variable/cam2!varName!label/value?value=!varValue!
)
